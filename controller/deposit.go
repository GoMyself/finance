...
func (that *DepositController) Offline(ctx *fasthttp.RequestCtx) {
	...
	data, err := model.DepositManualList(ex, startTime, endTime, page, pageSize)
	if err != nil {
		helper.Print(ctx, false, err.Error())
		return
	}
...
func (that *DepositController) OfflineToReview(ctx *fasthttp.RequestCtx) {
...
func (that *DepositController) OfflineReview(ctx *fasthttp.RequestCtx) {
