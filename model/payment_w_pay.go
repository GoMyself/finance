...
func (that *WPayment) Pay(orderId, ch, amount, bid string) (paymentDepositResp, error) {
	...
	v, err := httpDoTimeout("w pay", []byte(formData.Encode()), "POST", uri, header, time.Second*8)
	if err != nil {
...
func (that *WPayment) Withdraw(arg WithdrawAutoParam) (paymentWithdrawalRsp, error) {
	...
	v, err := httpDoTimeout("w pay", []byte(formData.Encode()), "POST", uri, headers, time.Second*8)
	if err != nil {
		return data, err
	}
